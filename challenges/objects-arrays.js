...
console.log(dinosaur[0].roar = "RAWERSRARARWERSARARARRRR!");
...
const unisWithUni = [];
/*for (let i = 0; i < graduates.length; i++) {
  const uni = unisWithUni.filter(uniWithUni => uniWithUni('Uni'));
}
...
lowCaseAnimalNames.map(animal_name => animal_name.toLowerCase);
...
lowPopulationAnimals.filter(lowPopulationAnimal => lowPopulationAnimal.population < 5);
...
function popTotal(population) {
  return population.reduce((acc, current) =>
  acc + current.population, 0)
}
